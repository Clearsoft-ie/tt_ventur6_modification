...
/*XML*/
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_accessories pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.accessories_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");

		foreach ($query->rows as $result) {
			$product_data[$result['accessories_id']] = $this->getProduct($result['accessories_id']);
		}
