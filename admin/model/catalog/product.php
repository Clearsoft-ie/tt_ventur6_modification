		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
				...
				/*XML*/
					...
					if (isset($data['product_accessories'])){
						foreach ($data['product_accessories'] as $accessories_id) {
							$this->db->query("DELETE FROM " . DB_PREFIX . "product_accessories WHERE product_id = '" . (int)$product_id . "' AND accessories_id = '" . (int)$accessories_id . "'");
							$this->db->query("INSERT INTO " . DB_PREFIX . "product_accessories SET product_id = '" . (int)$product_id . "', accessories_id = '" . (int)$accessories_id . "'");
						}
					}
				/*XML*/
	...
	public function copyProduct($product_id) {
		...
		if ($query->num_rows) {
			$data = $query->row;
				...
				/*XML*/
			if(VERSION > '2.0.3.1'){
				$data['product_accessories'] = $this->getProductaccessories($product_id);
			}else{
				$data = array_merge($data, array('product_accessories' => $this->getProductaccessories($product_id)));
			}
			/*XML*/
...
$this->Table();
	...
	public function getProductDiscounts($product_id) {
		...
		$product_accessories_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_accessories WHERE product_id = '" . (int)$product_id . "'");

		foreach ($query->rows as $result) {
			$product_accessories_data[] = $result['accessories_id'];
		}

		return $product_accessories_data;
	}
	
	public function Table(){
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX ."product_accessories` (`product_id` int(11) NOT NULL,`accessories_id` int(11) NOT NULL)");
	}
