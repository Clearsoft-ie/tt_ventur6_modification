...
/*XML*/
		$data['entry_accessories'] = $this->language->get('entry_accessories');
		$data['help_accessories'] = $this->language->get('help_accessories');
		
		if (isset($this->request->post['product_accessories'])) {
			$asproducts = $this->request->post['product_accessories'];
		} elseif (isset($this->request->get['product_id'])) {
			$asproducts = $this->model_catalog_product->getProductaccessories($this->request->get['product_id']);
		} else {
			$asproducts = array();
		}

		$data['product_accessoriess'] = array();

		foreach ($asproducts as $product_id){
			$accessoriess_info = $this->model_catalog_product->getProduct($product_id);

			if ($accessoriess_info) {
				$data['product_accessoriess'][] = array(
					'product_id' => $accessoriess_info['product_id'],
					'name'       => $accessoriess_info['name']
				);
			}
		}
